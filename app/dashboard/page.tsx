      ...
      <AuthGuard>
        <AppShell>
          <div className="space-y-8">
            ...
            {data.cowRegistrationAuthorizations?.some((authorization) => authorization.status === "PENDING_AUTHORIZATION") ? <section className="overflow-hidden rounded-3xl border border-amber-200 bg-gradient-to-br from-amber-50 via-white to-emerald-50 p-6 shadow-sm">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.2em] text-amber-700">Farmer authorization</p>
                  <h2 className="mt-2 text-xl font-semibold text-slate-950">Cow registration requests</h2>
                  <p className="mt-1 text-sm text-slate-600">Review each request carefully before sharing an authorization code.</p>
                </div>
                <div className="rounded-2xl bg-white/80 px-4 py-2 text-right ring-1 ring-amber-100">
                  <p className="text-xs uppercase tracking-wide text-slate-500">Requests</p>
                  <p className="text-xl font-bold text-slate-900">{data.cowRegistrationAuthorizations.filter((authorization) => authorization.status === "PENDING_AUTHORIZATION").length}</p>
                </div>
              </div>
              <div className="mt-4 space-y-3">
                {data.cowRegistrationAuthorizations.map((authorization) => <div key={authorization.batchId} className="rounded-2xl border border-amber-200 bg-white p-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <span className={`rounded-full px-3 py-1 text-xs font-semibold ${authorization.status === "EXPIRED" || authorization.status === "REJECTED" ? "bg-rose-100 text-rose-700" : authorization.status === "REGISTERED" || authorization.status === "AUTHORIZED" ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-800"}`}>{authorization.status === "EXPIRED" ? "Expired" : authorization.status === "REJECTED" ? "Rejected" : authorization.status === "REGISTERED" ? "Registered" : authorization.status === "AUTHORIZED" ? "Authorized" : "Pending authorization"}</span>
                  </div>
                  ...
                  {authorization.status === "PENDING_AUTHORIZATION" ? <div className="mt-4 grid gap-3 sm:grid-cols-1 md:grid-cols-3"><div className="rounded-xl bg-gradient-to-br from-amber-100 to-orange-100 p-4 ring-1 ring-amber-200"><p className="text-xs font-semibold uppercase tracking-wide text-amber-700">Private authorization OTP</p><p className="mt-1 text-3xl font-bold tracking-[0.3em] text-amber-950">{authorization.otpCode}</p><p className="mt-1 text-xs text-amber-800">Share only with the collector you trust.</p></div><div className="rounded-xl bg-slate-50 p-4 text-sm text-slate-700 ring-1 ring-slate-100"><p className="font-semibold">Authorization expires</p><p className="mt-1">{new Date(authorization.expiresAt).toLocaleString()}</p><p className="mt-1 text-xs text-slate-500">This code authorizes all {authorization.cowCount} cows in this request.</p></div><button type="button" onClick={() => { if (!window.confirm("Reject this cow registration request? The OTP will no longer work.")) return; void fetch("/api/system", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "rejectCowRegistrationBatch", userId: user?.uid, data: { batchId: authorization.batchId } }) }).then(async (response) => { if (!response.ok) { const result = await response.json() as { error?: string }; throw new Error(result.error ?? "Authorization rejection failed."); } window.location.reload(); }).catch((error: unknown) => setRequestMessage(error instanceof Error ? error.message : "Authorization rejection failed.")); }} className="rounded-xl border border-rose-200 bg-rose-50 p-4 text-sm font-semibold text-rose-700 transition hover:bg-rose-100">Reject request</button></div> : <div className={`mt-4 rounded-xl p-4 ${authorization.status === "REGISTERED" || authorization.status === "AUTHORIZED" ? "border border-emerald-200 bg-emerald-50" : "border border-rose-200 bg-rose-50"}`}><p className={`font-semibold ${authorization.status === "REGISTERED" || authorization.status === "AUTHORIZED" ? "text-emerald-800" : "text-rose-800"}`}>{authorization.status === "REJECTED" ? "Authorization rejected" : authorization.status === "REGISTERED" ? "Authorization completed successfully" : authorization.status === "AUTHORIZED" ? "Authorization approved" : "Authorization expired"}</p><p className={`mt-1 text-sm ${authorization.status === "REGISTERED" || authorization.status === "AUTHORIZED" ? "text-emerald-700" : "text-rose-700"}`}>{authorization.status === "REJECTED" ? "This request was rejected. Its OTP has been disabled." : authorization.status === "REGISTERED" ? "The OTP was accepted and these cows are now registered. The OTP is no longer available." : authorization.status === "AUTHORIZED" ? "This request was approved. The OTP is no longer available." : "This authorization window ended before verification. Ask the collector to submit a new request."}</p></div>}
                </div>)}
              </div>
            </section> : null}
