...
import type { Animal, BreedType, CowRegistrationAuthorization, Farmer, FarmerPayment, MilkBatch, MilkCollection, Phase2Summary, QualityTest, VeterinaryRecord } from "@/lib/phase2-data";
        ...
        <div className="space-y-6">
          ...
          <div className="grid gap-6 xl:grid-cols-2 print:hidden">
            ...
            <form className={`${!(currentUser.role === "MCC_OFFICER" ? (showSection("batches") || showSection("records")) : showSection("records")) || managementOnly || !canTestQuality ? "hidden " : ""}rounded-2xl border border-slate-200 bg-white p-5 shadow-sm`} onSubmit={(event) => { event.preventDefault(); requestSubmitConfirmation(currentUser.role === "MCC_OFFICER" ? "createBatchQualityTest" : "createQualityTest", { testId: `QT-${Date.now()}`, ...qualityForm, collectionId: currentUser.role === "MCC_OFFICER" ? undefined : qualityForm.collectionId, batchId: currentUser.role === "MCC_OFFICER" ? qualityForm.batchId : undefined, acidity: Number(qualityForm.acidity) || undefined, density: Number(qualityForm.density) || undefined, testedBy: currentUser.uid, testedAt: new Date().toISOString() }, "Quality result saved.", "quality result"); }}>
              ...
              <button disabled={!canWrite} className="mt-4 rounded-xl bg-emerald-600 px-4 py-2 text-sm font-medium text-white disabled:opacity-50">Save quality result</button>
            </form>

            <form className={`${!showSection("batches") || managementOnly || !canCreateBatch ? "hidden " : ""}rounded-2xl border border-slate-200 bg-white p-5 shadow-sm`} onSubmit={(event) => { event.preventDefault(); requestSubmitConfirmation("createBatch", { batchId: `BATCH-${Date.now()}`, mccId: farmerForm.mccId, batchDate: new Date().toISOString().slice(0, 10), totalLitres: Number(batchForm.totalLitres), destination: batchForm.destination, status: "OPEN", collectionIds: batchForm.collectionIds, createdBy: currentUser.uid, createdAt: new Date().toISOString() }, "Milk batch created.", "milk batch"); }}>
              <h2 className="text-lg font-semibold">Create milk batch</h2>
              {currentUser.role === "MILK_COLLECTOR" ? <p className="mt-1 text-sm text-slate-500">Select accepted collections from multiple farmers to form one batch.</p> : null}
              <div className="mt-4 grid gap-3 md:grid-cols-2">
              ...
              <button disabled={!canWrite} className="mt-4 rounded-xl bg-slate-900 px-4 py-2 text-sm font-medium text-white disabled:opacity-50">Create batch</button>
            </form>
